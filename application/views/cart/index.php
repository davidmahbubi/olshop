			...
			<table class="table mt-3">
				...
				<tbody>
					...
					<tr>
						<th scope="row"><?= $i++ ?></th>
						<td>
							<img src="<?=base_url()?>assets/img/product/<?=$c['image']?>" width="70" height="100"
								class="img-thumbnail d-inline-block mr-2" alt="" />
							<h5 class="d-inline"><?= $c['name'] ?></h5>
						</td>
						<td><?= formatPrice($c['price'], 'Rp') ?></td>
						<td>
							<input type="number" value="<?=$c['qty']?>" min="1" class="form-control w-100 itemTotal" id="cartInput" />
							<input type="hidden" class="rowid" value="<?=$c['rowid']?>">
						</td>
						<td><?= formatPrice($c['subtotal'], 'Rp'); ?></td>
						<td>
							<a href="<?=base_url()?>product/details/<?=$c['id']?>"
								class="btn  dv-bg-primary bt-dv-bg-primary text-white mr-2 mt-2">
								Details
							</a>
							<a onclick="return confirm('really want to delete this item from cart?');"
								href="<?=base_url()?>cart/removecartitem/<?=$c['rowid']?>" class="btn btn-outline-danger mr-2 mt-2">
								Delete
							</a>
						</td>
					</tr>
...
<div class="bottom w-100 position-fixed card">
	<div class="card-body">
		<div class="row">
			...
			<div class="col-md-4">
				<a href="<?=base_url()?>checkout" class="btn text-white dv-bg-primary bt-dv-bg-primary
              float-right">
					Checkout
				</a>
			</div>
