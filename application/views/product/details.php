					...
					<div class="col-lg-4">
						<form action="<?= base_url() ?>product/buy/<?= $product['id'] ?>" method="POST">
							<div class="input-group mb-3">
								...
								<input type="number" <?= $product['stock'] == 0 ? 'disabled' : '' ?> min="1" name="qty" value="1" class="form-control bt-cart" id="inputCart" placeholder="Buy Total" aria-label="Username" required>
							</div>
							<?php if (isLoggedIn()) : ?>
								<button type="button" <?= $product['stock'] == 0 ? 'disabled' : ''  ?> id="addToCartBt" class="btn w-100 btn-outline-primary mb-2 addToCart"><i class="fas fa-cart-plus cart-ic" style="font-size: 30px;"></i></button>
							<?php endif; ?>
							<button type="submit" <?= $product['stock'] == 0 ? 'disabled' : '' ?> class="btn text-white w-100 dv-bg-primary bt-dv-bg-primary" id="buyBt">Buy</button>
						</form>
...
<script>
	$(function() {
		...
		$('#inputCart').change(function() {
			if ($(this).val() <= 0) {
				console.log($(this));
				$('#addToCartBt').addClass('disabled').attr('disabled', 'disabled');
				$('#buyBt').addClass('disabled').attr('disabled', 'disabled');
			} else {
				$('#addToCartBt').removeClass('disabled').removeAttr('disabled', 'disabled');
				$('#buyBt').removeClass('disabled').removeAttr('disabled', 'disabled');
			}
		});
